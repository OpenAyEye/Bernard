...
def convert_lsb_to_g(lsb_values, conversion_factor=16384):
    """
    Convert LSB values to gravitational acceleration (g).

    Parameters:
    lsb_values (list or tuple): The LSB values for X, Y, and Z axes.
    conversion_factor (float): The LSB to g conversion factor (default: 16384).

    Returns:
    list: Acceleration in g for X, Y, and Z axes.
    """
    acceleration_g = [lsb / conversion_factor for lsb in lsb_values]
    return acceleration_g

def save_acceleration_data_to_file(accel_data, filename='accel_data.txt'):
    with open(filename, 'a') as file:
        file.write(f"{accel_data[0]:.4f} {accel_data[1]:.4f} {accel_data[2]:.4f}\n")


import time

def calibrate_accelerometer(icm20948, calibration_duration=60):
    calib_start_time = time.time()
    calib_data_x = []
    calib_data_y = []
    calib_data_z = []

    while time.time() - calib_start_time < calibration_duration:
        icm20948.icm20948_Gyro_Accel_Read()
        #Accel = icm20948.get_accel_data()  # Assuming you have a function to get accelerometer data

        calib_data_x.append(Accel[0])
        calib_data_y.append(Accel[1])
        calib_data_z.append(Accel[2])

        print(f"Calibration Gathering:\nX: {calib_data_x[-1]}\nY: {calib_data_y[-1]}\nZ: {calib_data_z[-1]}")
        time.sleep(0.1)

    calib_data_x = convert_lsb_to_g(calib_data_x)
    calib_data_y = convert_lsb_to_g(calib_data_y)
    calib_data_z = convert_lsb_to_g(calib_data_z)

    # Calculate the average for each axis
    calib_zero_x = sum(calib_data_x) / len(calib_data_x)
    calib_zero_y = sum(calib_data_y) / len(calib_data_y)
    calib_zero_z = sum(calib_data_z) / len(calib_data_z)
    print(f"Calibrated_zero_x: {calib_zero_x} \n Calibrated_zero_y: {calib_zero_y} \n Calibrated_zero_z: {calib_zero_z}\n")
    return calib_zero_x, calib_zero_y, calib_zero_z
...
if __name__ == "__main__":
    ...
    c = BluetoothClient("pi-desktop", data_received)
    try:
        ...
        icm20948 = ICM20948()
        x_zero, y_zero, z_zero = calibrate_accelerometer(icm20948)
        threshold = 0.1
        move_state = 0
        while True:
            icm20948.icm20948_Gyro_Accel_Read()
            ...
            acceleration_g = convert_lsb_to_g(Accel)
            current_time = datetime.datetime.now()
            # Calculate the absolute difference between the current acceleration and the calibrated values
            diff_x = abs(acceleration_g[0] - x_zero)
            diff_y = abs(acceleration_g[1] - y_zero)
            diff_z = abs(acceleration_g[2] - z_zero)
            print(
                f"time: {current_time} :: Acceleration in g: X = {acceleration_g[0]}, Y = {acceleration_g[1]}, Z = {acceleration_g[2]}")
            ...
            print(
                f"time: {current_time} :: Acceleration in g: X = {acceleration_g[0]}, Y = {acceleration_g[1]}, Z = {acceleration_g[2]}")
        # Simulate data retrieval 6 to 8 times a second

            # Adjust the sleep time to control the data update rate
            time.sleep(1)
        ...
        # Close the Bluetooth connection when the script ends
